namespace LiskovSubstitution_BadDesign
{
    ...
    {
        // A 'good example' on Liskov substitution is simply to not allow our sub-classes to differ in the meaning of their implementations. I.e. no explicit paths for certain sub-types, no strenghtening of conditions etc. If a need to break away from limitations of the originals class, we can add the required functionality to that original class.

        // By introducing an abstract base-class, which takes into consideration the diverse implementations, we can ensure sub-class compatibility. Given additional requirements - for example, we only need to modify the base-class, single point of maintainability.
